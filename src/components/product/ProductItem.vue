<template>
  <section id="product-item" class="box" v-if="productItem">
    ...
    <div class="product-item__image">
      <img :src="`../src/assets/${productItem.image_tag}`" />
    </div>
  </section>
</template>
...
const productItem = computed(() => {
  return store.getters.productItemFromId(Number(props.id));
});

const addAndGoToCart = (item) => {
  store.dispatch('addCartItem', item).then(() => {
    router.push('/cart');
  });
};
</script>
